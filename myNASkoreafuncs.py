...
class crawlerKorea():
    def __init__(self):
        ...
        self.cc = self.cc = OpenCC('s2twp')  # opencc.Converter(from_variant='cn', to_variant='twp')
    ...
    def initAccumulatedCasterJson(self):
        ...
        if os.path.exists(accumulatedCasterJson) and os.path.isfile(accumulatedCasterJson):
            ...
            logging.info('=== 已將已存在的[%s]檔案，更改名稱為[%s] ===' % (self.accumulatedCasterJson, accumulatedCasterJson))
            existingFile = True
    ...
    def recoverfromAccumulatedCasterJson(self):
        ...
        def openAccumulatedCasterJson():
            ...
            try:
                ...
                try:
                    ...
                    logging.warning('=== 在開啟檔案 %s 遇到錯誤 [json.decoder.JSONDecodeError] 已試圖修復檔案後仍有錯誤 [%s] ===' % (self.accumulatedCasterJson, str(e)))
                else:
                    if self.accumulatedCaster:
                        return True
            except Exception as e:
                logging.warning('=== 使用方法開啟 %s 檔案錯誤! 錯誤訊息: [%s] ===' % (self.accumulatedCasterJson, str(e)))
                return False
            else:
                if self.accumulatedCaster:
                    return True         
        if os.path.isfile(self.accumulatedCasterJson):
            ...
            logging.error('=== 沒有可復原資料的檔案 %s 可以復原 ===' % self.accumulatedCasterJson)
            return False
    ...
    def finilizeAccumulatedCasterJson(self):
        try:
            ...
            logging.error('=== 儲存當天資料檔 %s 錯誤, 失敗原因: %s ===' % (self.accumulatedCasterJson, str(e)))             
    ...
    def downloadProcess(self, ds214se):
        ...
        for self.casterIndexer in range(self.givenNo2Start, self.givenNo2Stop):
            ...
            # ------ 找出主播序號 -----------------------------   
            for i in range(len(caster)-1, -1, -1):
                if not caster[i].isdigit():
                    folderNumberstr = caster[i+1:]
                    break
            # ------ 主播下層主資料夾為名字和序號做尾碼 -----------------------------  
            ...
            if os.path.isdir(os_subFolder):  # ------ 資料夾已存在
                ...
                if ds214se.createFolder(ds_subFolder):
                    logging.debug('=== [%s] 建立子資料夾 [%s] 成功 ===' % (caster, ds214se.folder))
                else:
                    logging.error('=== [%s] 建立子資料夾 [%s] 失敗 ===' % (caster, ds214se.folder))
                    return False
            logging.info('=== 下層子資料夾位置: [%s] ===' % ds214se.folder)   
            # ------ 主播_影像檔 -----------------------------
            content = 'casterZip'
            casterZip = task[content].split('/')[-1].lower()
            if casterZip:
                if folderNumberstr != casterZip.rstrip('.zip'):
                    logging.error('=== 錯誤 [%s] 資料夾名稱和號碼 [%s] 對應不起來 ===' % (caster, subFolder))
                    return False
                if os.path.isfile('%s/%s' % (os_subFolder, casterZip)):  # ------ 檔案已存在
                    logging.debug('=== 此 [%s] 影像檔 [%s] 已下載過 ===' % (caster, casterZip))
                else:  # ------ 檔案不存在
                    if not urllib.parse.urlparse(task[content]).scheme:
                        task[content] = self.appScheme + task[content]                    
                    if ds214se.createTask(task[content].replace(' ', r'%20')):
                        logging.info('=== 建立下載 [%s] 影像檔 [%s] 成功 ===' % (caster, task[content]))           
                    else:
                        logging.error('=== 建立下載 [%s] 影像檔 [%s] 失敗 ===' % (caster, task[content]))
                        return False
            else:
                logging.warning('=== 注意，第 [%d] 筆資料 casterZip [%s] 似乎有問題 ===' % (self.casterIndexer, casterZip))
            # ------ 主播_頭像檔 -----------------------------
            content = 'thumbnail'
            thumbnail = task[content].split('/')[-1].lower()
            if thumbnail:
                if os.path.isfile('%s/%s' % (os_subFolder, thumbnail)):
                    logging.info('=== 此 [%s] 頭像檔 [%s] 已下載過 ===' % (caster, task[content]))
                else:
                    if not urllib.parse.urlparse(task[content]).scheme:
                        task[content] = self.appScheme + task[content]                               
                    if ds214se.createTask(task[content].replace(' ', r'%20')):
                        logging.info('=== 建立下載 [%s] 頭像檔 [%s] 成功 ===' % (caster, task[content]))
                    else:
                        logging.error('=== 建立下載 [%s] 頭像檔 [%s] 失敗 ===' % (caster, task[content]))
                        return False
            else:
                logging.warning('=== 注意，第 [%d] 筆資料內容 [%s] 似乎有問題 ===' % (self.casterIndexer, content))                       
            # ------ 主播_圖像檔 -----------------------------
            content = 'casterImg'
            casterImg = task[content].split('/')[-1].lower()
            if casterImg:
                if os.path.isfile('%s/%s' % (os_subFolder, casterImg)):
                    logging.info('=== 此 [%s] 圖像檔 [%s] 已下載過 ===' % (caster, task[content]))
                else:
                    if not urllib.parse.urlparse(task[content]).scheme:
                        task[content] = self.appScheme + task[content]                
                    if ds214se.createTask(task[content].replace(' ', r'%20')):
                        logging.info('=== 建立下載 [%s] 圖像檔 [%s] 成功 ===' % (caster, task[content]))
                    else:
                        logging.error('=== 建立下載 [%s] 圖像檔 [%s] 失敗 ===' % (caster, task[content]))            
                        return False
            else:
                logging.warning('=== 注意，第 [%d] 筆資料內容 [%s] 似乎有問題 ===' % (self.casterIndexer, content))                              
        return True

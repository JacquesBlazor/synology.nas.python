...
class crawlerKorea():
    ...
    def initAccumulatedCasterJson(self):
        ...
        if os.path.exists(accumulatedCasterJson) and os.path.isfile(accumulatedCasterJson):
            ...
            try:
                ...
                logging.error('=== 在更改資料檔 %s 為 %s 時錯誤, 失敗原因: %s ===' % (self.accumulatedCasterJson, accumulatedCasterJson, str(e)))
            else:    
        ...
        try:
            with open(self.accumulatedCasterJson, 'r+b') as f:
                ...
                if removeComma == b'},':
                    ...
                    logging.warning('=== 沒有將 accumulatedCasterJson 正確結尾: % ' % removeComma)
        except Exception as e:
            logging.error('=== 儲存當天資料檔 %s 錯誤, 失敗原因: %s ===' % (self.accumulatedCasterJson, str(e)))             
    ...
    # ------ 開始執行 --------------------------  
    def downloadProcess(self, diskStation):
        accumulateCounter = 0
        ...
        for self.casterIndexer in range(self.givenNo2Start, self.givenNo2Stop):
            ...
            logging.debug('=== 目前擷取第[%d]個_序號[%d] ===' % (accumulateCounter, self.casterIndexer))
            # ------ 主播上層主資料夾 --------------------------       
            ...
            if os.path.isdir(os_folder):  # ------ 資料夾已存在
                ...
                if diskStation.createFolder(ds_folder):
            ...
            if os.path.isdir(os_subFolder):  # ------ 資料夾已存在
                ...
                if diskStation.createFolder(ds_subFolder):
                    logging.debug('=== [%s] 建立子資料夾 [%s] 成功 ===' % (caster, diskStation.folder))
                else:
                    logging.error('=== [%s] 建立子資料夾 [%s] 失敗 ===' % (caster, diskStation.folder))
                    return False 
            # ------ 主播_檔案s -----------------------------
            logging.info('===[ 第 %d 位主播 %s，序號：%d，資料夾： %s/%s ]===' % (accumulateCounter, caster, self.casterIndexer, self.userbaseDir, diskStation.folder))             
            downloadUris = []
            for contentKey in contentKeys: 
                contentValue = task[contentKey].split('/')[-1].lower()
                if contentValue:
                    if os.path.isfile('%s/%s' % (os_subFolder, contentValue)):  # ------ 檔案已存在
            ...
            # ------ 下載主播的檔案 -----------------------------
            if downloadUris:
                if diskStation.createTask(','.join(downloadUris)):
                    logging.info('=========[ 主播 %s 共 %s 檔案影像檔, 已成功建立下載清單 ]=========' % (caster, len(downloadUris)))
                else:
                    logging.error('=========[ 主播 %s 共 %s 檔案影像檔, 建立下載清單時失敗 ]=========' % (caster, len(downloadUris)))
                    return False
            else:
                logging.info('=========[ 第 %d 位主播 %s，序號：%d，沒有檔案需要下載 ]=========' % (accumulateCounter+1, caster, self.casterIndexer)) 
        return True
...
if __name__ == '__main__':
    logging.basicConfig(filename='%s/.scheduler/.log/.dailyCrawlClean/%s.log' % (os.path.expanduser('~'), datetime.now().strftime('%Y%m%d_%H%M%S')), level=logging.INFO, format='%(asctime)s - %(levelname)s: %(message)s')
    # --- 先啟始 LINE 通知的物件 -----------------------------       
    ...
    # --- 讀取流程設定檔 ----------------------------- 
    try:
        dailyCrawlCleanConfig = None
        dailyCrawlerConf = '/volume1/scheduler/dailyCrawlClean.config'        
        with open(dailyCrawlerConf, 'r', encoding='utf-8') as f:
            dailyCrawlCleanConfig = json.load(f)
    except FileNotFoundError:
        myLineNotificator.sendMessage('沒有設定檔案: %s。格式為: {"flag": 1}' % dailyCrawlCleanConfig)
        sys.exit(1)
    if dailyCrawlCleanConfig == None:
        myLineNotificator.sendMessage('設定檔案: %s 沒有內容。格式為: {"flag": 1}' % dailyCrawlCleanConfig)
        sys.exit(1)
    # --- 初始化 myKoreaCrawler 物件 -----------------------------
    if dailyCrawlCleanConfig['myKoreaCrawler']:
    ...
    if dailyCrawlCleanConfig['myKoreaCrawlerSession']:
    ...
    if dailyCrawlCleanConfig['myKoreaCrawlerGetJust']:    
        ...
        # --- 無法讀入資料檔 -----------------------------
            ...
            sys.exit(1)
    # --- 如果正常登入網站則讀入昨天的資料檔 -----------------------------
    if dailyCrawlCleanConfig['myKoreaCrawlerGetLast']:
    ...
    if dailyCrawlCleanConfig['myKoreaCrawlerRecovery']:               
    ...
    # --- 啟始當天的記錄檔開始新的蒐集記錄 -----------------------------
    if dailyCrawlCleanConfig['myKoreaCrawlerInitialize']:   
    ...
    # --- 如果設定開始序號有設定 ---
    if dailyCrawlCleanConfig['myKoreaCrawlerStartStop']:   
        ...
        # --- 如果沒有設定開始序號(應該不會發生) -----------------------------  
            ...
            sys.exit(1)
    # --- 看來一切就緒我們就開始蒐集資料吧 -----------------------------
    if dailyCrawlCleanConfig['myKoreaCrawlerStartCrawl']: 
    ...
    if dailyCrawlCleanConfig['pttBeautyCrawler']: 
    ...
    if dailyCrawlCleanConfig['pttBeautyCrawlerGetJust']:    
    ...
    # ---- 啟始記錄檔 --------------------------
    if dailyCrawlCleanConfig['pttBeautyCrawlerInitialize']: 
    ...
    # --- 看來一切就緒我們就開始蒐集資料吧 -----------------------------
    if dailyCrawlCleanConfig['pttBeautyCrawlerStartCrawl']: 
    ...
    if dailyCrawlCleanConfig['ds214se']:
    ...
    if dailyCrawlCleanConfig['ds214seDownloadMyKorea']:
    ...
    if dailyCrawlCleanConfig['ds214seDownloadPttBeauty']:
...
'''
dailyCrawlClean.config 的預設值為:
{ 
  "myKoreaCrawler": 1, 
  "myKoreaCrawlerSession": 1, 
  "myKoreaCrawlerGetJust": 1,
  "myKoreaCrawlerGetLast": 0, 

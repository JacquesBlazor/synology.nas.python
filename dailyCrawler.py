...
if __name__ == '__main__':
    ...
    # --- 讀取流程設定檔 ----------------------------- 
    try:
        dailyCrawlerConfig = None
        dailyCrawlerConf = '/volume1/scheduler/dailyCrawler.config'        
        with open(dailyCrawlerConf, 'r', encoding='utf-8') as f:
            dailyCrawlerConfig = json.load(f)
    except FileNotFoundError:
        myLineNotificator.sendMessage('沒有設定檔案: %s。格式為: {"flag": 1}' % dailyCrawlerconfig)
        sys.exit(1)
    if dailyCrawlerConfig == None:
        myLineNotificator.sendMessage('設定檔案: %s 沒有內容。格式為: {"flag": 1}' % dailyCrawlerconfig)
        sys.exit(1)
    # --- 初始化 myKoreaCrawler 物件 -----------------------------
    if dailyCrawlerConfig['myKoreaCrawler']:
    ...
    if dailyCrawlerConfig['myKoreaCrawlerSession']:
    ...
    if dailyCrawlerConfig['myKoreaCrawlerGetJust']:    
        ...
        # --- 無法讀入資料檔 -----------------------------
            ...
            sys.exit(1)
    # --- 如果正常登入網站則讀入昨天的資料檔 -----------------------------
    if dailyCrawlerConfig['myKoreaCrawlerGetLast']:
    ...
    if dailyCrawlerConfig['myKoreaCrawlerRecovery']:               
    ...
    # --- 啟始當天的記錄檔開始新的蒐集記錄 -----------------------------
    if dailyCrawlerConfig['myKoreaCrawlerInitialize']:   
    ...
    # --- 如果設定開始序號有設定 ---
    if dailyCrawlerConfig['myKoreaCrawlerStartStop']:   
        ...
        # --- 如果沒有設定開始序號(應該不會發生) -----------------------------  
            ...
            sys.exit(1)
    # --- 看來一切就緒我們就開始蒐集資料吧 -----------------------------
    if dailyCrawlerConfig['myKoreaCrawlerStartCrawl']: 
    ...
    if dailyCrawlerConfig['pttBeautyCrawler']: 
    ...
    if dailyCrawlerConfig['pttBeautyCrawlerGetJust']:    
    ...
    # ---- 啟始記錄檔 --------------------------
    if dailyCrawlerConfig['pttBeautyCrawlerInitialize']: 
    ...
    # --- 看來一切就緒我們就開始蒐集資料吧 -----------------------------
    if dailyCrawlerConfig['pttBeautyCrawlerStartCrawl']: 
    ...
    if dailyCrawlerConfig['ds214seDownloaMyKorea']:
    ...
    if dailyCrawlerConfig['ds214seDownloaPttBeauty']:

# --- The MIT License (MIT) Copyright (c) alvinlin, Fri May 2nd 02:55:00 2020 ---
...
def appProcedure():
    ...
    logging.debug('全部數量: %s 個. ' % listedTasksTotal)
    if downloadingTasks:
        logging.debug('下載中: %s 個. ' % len(downloadingTasks))            
        logging.debug(downloadingTasks)
    if finishedTasks:
        logging.debug('已完成: %s 個. ' % len(finishedTasks))
        logging.debug(finishedTasks)
    if finishingTasks:
        logging.debug('正在完成: %s 個. ' % len(finishingTasks))
        logging.debug(finishingTasks)        
    if pausedTasks:
        logging.debug('暫停中: %s 個. ' % len(pausedTasks))
        logging.debug(pausedTasks)
    if waitingTasks:
        logging.debug('等待中: %s 個. ' % len(waitingTasks))
        logging.debug(waitingTasks)             
    if errorTasks:
        logging.debug('有錯誤: %s 個. ' % len(errorTasks))            
        logging.debug(errorTasks)
    if otherTasks:
        logging.debug('其他未知: %s 個. ' % len(otherTasks))            
        logging.debug(otherTasks)
...
if __name__ == '__main__':
    ...
    if nasConfig:
        ...
        if ds214se.login:      
            ...
            # ------ 主要功能 ------
            nasDownloading = appProcedure()
            from pprint import pprint
            pprint(nasDownloading)
            # ------ 登出 ------

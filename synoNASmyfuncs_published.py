# --- The MIT License (MIT) Copyright (c) alvinconstantine(alvin.constantine@outlook.com), Wed May 6 10:20:00am 2020 ---
...
# ------ 建立從 DownloadStation 的 listTask 結果來分類(catagorize)並計算下載項目效率的函式  -----------------------------
def get_catagorizedDict():
    ...
    if listedTasks:
        logging.info('全部數量: %s 個. ' %  len(listedTasks))
        downloadingTasks = list(filter(lambda i: i['status'] == 'downloading', listedTasks))
        if downloadingTasks:
        ...
        finishedTasks = list(filter(lambda i: i['status'] == 'finished', listedTasks))
        if finishedTasks:
            logging.info('已完成: %s 個. ' % len(finishedTasks))
            logging.debug(finishedTasks)
        finishingTasks = list(filter(lambda i: i['status'] == 'finishing', listedTasks))
        if finishingTasks:
            logging.info('正在完成: %s 個. ' % len(finishingTasks))
            logging.debug(finishingTasks)    
        pausedTasks = list(filter(lambda i: i['status'] == 'paused', listedTasks))
        if pausedTasks:
            logging.info('暫停中: %s 個. ' % len(pausedTasks))
            logging.debug(pausedTasks)                
        waitingTasks = list(filter(lambda i: i['status'] == 'waiting', listedTasks))
        if waitingTasks:
            logging.info('等待中: %s 個. ' % len(waitingTasks))
            logging.debug(waitingTasks)                        
        errorTasks = list(filter(lambda i: i['status'] == 'error', listedTasks))
        if errorTasks:
            logging.info('有錯誤: %s 個. ' % len(errorTasks))            
            logging.debug(errorTasks)                
        del listedTasks
        listedTasksinDict = {}
        listedTasksinDict ['downloading'] = downloadingTasks
        listedTasksinDict ['finished'] = finishedTasks
        listedTasksinDict ['finishing'] = finishingTasks
        listedTasksinDict ['paused'] = pausedTasks
        listedTasksinDict ['waiting'] = waitingTasks
        listedTasksinDict ['error'] = errorTasks
        return listedTasksinDict 
    else:
        return None 
...
if __name__ == '__main__':
    ...
    if nasConfig:
        ...
        if ds214se.login:      
            ...
            # ------ 主要功能 -----------------------------     
            catagorizedDict = get_catagorizedDict()
            if catagorizedDict:
                if catagorizedDict['downloading']:
                    for i, j in enumerate(catagorizedDict['downloading']):
                        print('%s. 項目: [%s], 效率: [%s].' % (i+1, j['title'], j['ratio']))
            # ------ 登出 ----------------------------- 

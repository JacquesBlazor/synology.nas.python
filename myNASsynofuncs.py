...
# ------ 建立 NAS 中 DiskStation 的 Storage 方法 -------------------------------
    def Storage(self, method, **kwargs):
        ...
        if isinstance(method, str) and method.lower() in ('load_info', 'get_usage'):
            ...
            return False
        # ------ 建立 NAS 中 DiskStation 的 Storage 的 load_info 方法 -------------------------------    
        ...
        # ------ 建立 NAS 中 DiskStation 的 Storage 的 get_usage 方法 -------------------------------           
...
# ------ 建立 NAS 中 FileStation 的方法 -------------------------------
    def FileStation(self, method, **kwargs):
        self.name = 'FileStation'
        if isinstance(method, str) and method.lower() in ('create'):
            self.method = method.lower()
            logging.debug('=== 開始執行: %s, %s 作業 ===' % (self.name, self.method))
        else:
            logging.error('=== 此 %s 中沒有指定的 %s 作業方法 ===' % (self.name, str(method)))
            return False
        # ------ 建立 NAS 中 FileStation 的 create 方法  -----------------------------

# --- The MIT License (MIT) Copyright (c) alvinconstantine(alvin.constantine@outlook.com), Tue Jun 16 18:48:00pm 2020 ---
    ...
    def FileStation(self, method, **kwargs):
        ...
        if self.method == 'create':
            ...
            if data in kwargs:                                   
                ...
                logging.debug('=== 正在執行: %s, %s 作業 ===' % (self.name, self.method))
                logging.debug('=== 參數: %s, %s ===' % (str(self.crtfdr_url), str(self.crtfdr_params)))
                try:              
                    response = requests.get(self.crtfdr_url, params=self.crtfdr_params).json()
                except TimeoutError:
                    logging.error('=== 連線的主機 %s:%d 沒有回應，連線嘗試失敗 ===' % (self.ip, self.port))
                    return False
                else:
...
# ------ 建立 NAS 中 DownloadStation 的 share param 方法 -----------------------------
    def DownloadStation(self, method, **kwargs):
        # ------ 開始建立 NAS 中 DownloadStation 的主方法 -----------------------------

# --- The MIT License (MIT) Copyright (c) alvinconstantine(alvin.constantine@outlook.com), Tue Jun 19 10:21:00am 2020 ---
                ...
                if self.incompletedArticles:                 
                    ...
                    while self.incompletedArticles:
                        ...
                        if respPage:
                            ...
                            sleepLonger += 2
                        break
                logging.info('===[ 已完成 %s 日的文章抓取，網址: %s 已沒有當天資料 ]===' % (self.timeYesterday, self.currentPage))
                break
        return True
    ...
    def downloadProcess(self, diskStation, **kwargs):
        ...
        for self.beautyIndexer, task in enumerate(self.accumulatedArticles):
            ...
            if not task['img_urls']:
                ...
                logging.info('=== 資料夾名稱為 [%s] ===' % ds_folder)         

...
class crawlerPttBeauty():
    def __init__(self):
        ...
        self.completedBeautyPickle = 'completedBeautyPickle_%s.pickle' % self.formatYesterday    
    ...
    def initAccumulatedBeautyJson(self):
        ...
        if os.path.exists(accumulatedBeautyJson) and os.path.isfile(accumulatedBeautyJson):
            ...
            try:
                ...
                logging.error('=== 更改資料檔 %s 為 %s 錯誤, 失敗原因: %s ===' % (self.accumulatedBeautyJson, accumulatedBeautyJson, str(e)))
        try:
    ...
    def downloadProcess(self, diskStation, **kwargs):
        ...
        for self.beautyIndexer, task in enumerate(self.accumulatedArticles):
            if not task['title']:
                ...
                logging.warning('=== 使用 [%s] 作為文章標題名稱 ===' % task['title']) 
            ...
            if not task['img_urls']:
                ...
                if os.path.isdir('%s/%s' % (self.userbaseDir, ds_folder)):  # ------ 資料夾已存在
                    ...
                    if diskStation.createFolder(ds_folder):
                        ...
                        return False
                logging.info('=== 資料夾位置: [%s] ===' % diskStation.folder)                
                # ------ 表特_影像檔 -----------------------------
                for img in task['img_urls']:
                    if os.path.isfile('%s/%s/%s' % (self.userbaseDir, ds_folder, os.path.basename(img))):  # ------ 檔案已存在
                        logging.debug('=== 此 [%s] 影像檔 [%s] 已下載過 ===' % (img, os.path.basename(img)))
                    else:  # ------ 檔案不存在                
                        if diskStation.createTask(img):

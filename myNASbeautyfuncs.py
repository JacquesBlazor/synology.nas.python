    ...
    def downloadProcess(self, diskStation, **kwargs):
        ...
        for self.beautyIndexer, task in enumerate(self.accumulatedArticles):
            if not task['title']:
                ...
                strb4colon = task['title'][:task['title'].find(':')+1]
                task['title'] = task['title'].replace(strb4colon, '')
                logging.warning("=== 文章標題名稱 [%s] 已被移除。目前以 [%s] 作為文章標題名稱 ===" % (strb4colon, task['title']))
            if not task['img_urls']:
                ...
                for index, img in enumerate(task['img_urls']):
                    ...
                    if os.path.isfile('%s/%s/%s' % (self.userbaseDir, ds_folder, imgFile)):  # ------ 檔案已存在                        
                        ...
                        img_urls.append(img)
                if img_urls:
                    if diskStation.createTask(','.join(img_urls)):
                        logging.info('=========[ 成功|文章 %s |已新增 %d 項影像表列到下載清單 ]=========' % (task['title'], len(img_urls)))
                    else:
                        logging.error('=========[ 失敗|文章 %s |未新增 %d 項影像表列到下載清單 ]=========' % (task['title'], len(img_urls)))
                        return False
                else:
